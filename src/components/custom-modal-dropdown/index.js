...
export default class CustomModalDropdown extends Component {
  ...
  _calcPosition() {
    const {dropdownStyle, style, adjustFrame, flexWidth, flexHeight} = this.props;

    const dimensions = Dimensions.get('window');
    const windowWidth = flexWidth?flexWidth: dimensions.width;
    const windowHeight = flexHeight?flexHeight:dimensions.height;

    const dropdownHeight = (dropdownStyle && StyleSheet.flatten(dropdownStyle).height) ||
      StyleSheet.flatten(styles.dropdown).height;

    const bottomSpace = windowHeight - this._buttonFrame.y - this._buttonFrame.h;
    const rightSpace = 0;//windowWidth - this._buttonFrame.x;
    const showInBottom = bottomSpace >= dropdownHeight || bottomSpace >= this._buttonFrame.y;
    const showInLeft = true;//rightSpace >= this._buttonFrame.x;
  
    const positionStyle = {
      height: dropdownHeight,
      width: flexWidth?flexWidth:(windowWidth-40) ,
    ...
    if (TOUCHABLE_ELEMENTS.find(name => name == row.type.displayName)) {
      ...
      switch (row.type.displayName) {
        case 'TouchableHighlight': {
          return (
            <TouchableHighlight {...props} underlayColor={"#d4d4d4"}>
              {children}
            </TouchableHighlight>
          );
        }
        case 'TouchableOpacity': {
          return (
            <TouchableOpacity {...props} underlayColor={"#d4d4d4"}>
              {children}
            </TouchableOpacity>
          );
        }
        case 'TouchableWithoutFeedback': {
          return (
            <TouchableWithoutFeedback {...props} underlayColor={"#d4d4d4"}>
              {children}
            </TouchableWithoutFeedback>
          );
        }
        case 'TouchableNativeFeedback': {
          return (
            <TouchableNativeFeedback {...props} underlayColor={"#d4d4d4"}>
              {children}
            </TouchableNativeFeedback>
          );
        }
